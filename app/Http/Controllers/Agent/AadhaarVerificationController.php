...
class AadhaarVerificationController extends Controller
{
    ...
    function sendOTP(Request $request)
    {
        ...
        if (Auth::User()->isAadharVerify == 1) {
            return Response()->json(['status' => 'failure', 'message' => 'Aadhar number already verified']);
        }
        ...
        //$response = '{"status":"success","message":"OTP sent successfully","client_id":"M1737010556","reference_id":"28469133"}';
    ...
    function confirmOtpWeb(Request $request)
    {
        $rules = array(
        ...
        $mode = "WEB";
        $user_id = Auth::id();
        return Self::confirmOtpMiddle($aadhar_aumber, $client_id, $reference_id, $otp, $mode, $user_id);
    }

    function confirmOtpMiddle($aadhar_aumber, $client_id, $reference_id, $otp, $mode, $user_id)
    {
        ...
        if ($status === 'success') {
            ...
            if ($data->status == 'VALID') {
                ...
                Member::where('user_id', $user_id)->update([
                    ...
                    'pin_code' => $data->split_address->pincode,
                    'office_address' => $data->address,
                    'aadhar_number' => $aadhar_aumber,
                    'care_of' =>  $data->care_of,
                    'dob' => $data->dob,
                    'gender' => $data->gender,
                    'aadhar_profile_photo' => $profile_photo,
                ]);
                return Response()->json(['status' => 'success', 'message' => 'Aadhaar verification successfully complete!']);
            } else {
    ...
    function uploadAadharImage($image, $name)
    {
        ...
        \File::put(public_path('profile-pic') . '/' . $imageName, base64_decode($image));
        return $imageName;
    }
}
