...
class PunjikendraController extends Controller
{
    ...
    function welcome()
    {
        ...
        if ($serviceStatus == 1) {
            $data = array('page_title' => 'Payin 4');
            return view('agent.add-money.punjikendra')->with($data);
        } else {
    ...
    function createOrderMiddle($amount, $user_id, $mode, $callback_url, $client_id, $name, $email, $mobile)
    {
        ...
        if ($serviceStatus == 1) {
            ...
            $url = $this->base_url . 'qrupipay/createqrp';
            $parameters = array(
                'Apikey' => $this->api_key,
                'amount' => $amount,
                'url' => url('api/call-back/punjikendra'),
                'name' => $name,
                'email' => $email,
            ...
            if ($status == true) {
                ...
                $qrCodeUrl = url('agent/add-money/v4/view-qrcode') . '?upi_string=' . urlencode($qrString);
                $data = [
                    'qrCodeUrl' => $qrCodeUrl,
                    'qrString' => $qrString,
                    'txnid' => $orderId,
    ...
    function callbackUrl(Request $request)
    {
        ...
        /* $rules = array(
             'txnstatus' => ['required', 'string', 'in:Success,Failure'],
             'partnerTxnId' => ['required', 'integer', 'exists:gatewayorders,id'],
             'amount' => ['required', 'numeric', 'min:1'],
             'rrn' => ['required', 'string', 'max:255', 'regex:/^[a-zA-Z0-9]+$/'],
         );
         $validator = Validator::make($request->all(), $rules);
         if ($validator->fails()) {
             return Response()->json(['status' => 'failure', 'message' => $validator->messages()->first()]);
         }*/
        /*  $clientIp = '164.52.197.226';
        ...
        */
        ...
        $orderId = $data['partnerTxnId'] ?? null;
        ...
        if ($txnstatus == 'Success') {
            ...
            if ($gatewayorders) {
                ...
                $reports = Report::where('id', $orderId)->first();
                if ($reports) {
                    $id = $reports->id;
                    $mode = "Call-back";
                    $reports = Report::find($id);
                    if ($reports->wallet_type == 1 && $txnstatus == 'Success') {
                        $library = new RefundLibrary();
                        return $library->update_transaction($status = 1, $utr, $id, $mode);
                    }
                }
