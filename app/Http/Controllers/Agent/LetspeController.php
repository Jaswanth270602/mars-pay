...
use App\Library\LetspeLibrary;

class LetspeController extends Controller
{
    ...
    function welcome()
    {
        ...
        if ($serviceStatus == 1) {
            ...
            return view('agent.add-money.letspe')->with($data);
        } else {
    ...
    function createOrderMiddle($amount, $user_id, $mode, $callback_url, $client_id)
    {
        ...
        if ($serviceStatus == 1) {
            ...
            $library = new LetspeLibrary();
            return $response = $library->createOrders($orderId, $amount, $mobile_number, $username, $email_id);
        } else {
